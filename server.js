...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.articleId && database.articles[requestComment.articleId]
      && requestComment.username && database.users[requestComment.username] &&requestComment.body.length > 0) {
    const comment = {
...
function updateComment(url, request) {
  ...
  if (!savedComment) {
    response.status = 404;
  } else if (!id || (id!==requestComment.id) || requestComment.body.length <= 0){
    response.status = 400;
  }
  else {
    response.body = {comments: requestComment};
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    database.users[savedComment.username].commentIds = userCommentIds;
    ...
    database.articles[savedComment.articleId].commentIds = userCommentIds;
    response.status = 204;
  } else {
    response.status = 404;
  }
